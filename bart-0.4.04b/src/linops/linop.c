...
struct linop_s* linop_chainAdj(const struct linop_s* a, const struct linop_s* b)
{
	PTR_ALLOC(struct linop_s, c);
...
struct linop_s* linop_chainNormal(const struct linop_s* a, const struct linop_s* b)
{
	PTR_ALLOC(struct linop_s, c);
...
struct linop_s* linop_PutFowrardOfBInNormalOfA(const struct linop_s* a, const struct linop_s* b)
{
	PTR_ALLOC(struct linop_s, lo);
...
/*
struct linop_s* linop_PutNormalOfBInNormalOfA(const struct linop_s* a, const struct linop_s* b)
{
	PTR_ALLOC(struct linop_s, lo);

	lo->forward = operator_ref(a->forward);
	lo->adjoint = operator_ref(a->adjoint);
	lo->normal = operator_ref(b->normal);
	lo->norm_inv = operator_p_ref(a->norm_inv);
