...
static void sampling_apply(const linop_data_t* _data, complex float* dst, const complex float* src)
{
	...
	debug_printf(DP_INFO,"sampling_apply\n");
	debug_printf(DP_INFO,"data->dims:");
	debug_print_dims(DP_INFO,DIMS,data->dims);
	debug_printf(DP_INFO,"data->pat_dims:");
	debug_print_dims(DP_INFO,DIMS,data->pat_dims);
}
...
struct linop_s* linop_samplingGeneral_create(const long dims[DIMS], const long pat_dims[DIMS], const complex float* pattern)
{
	PTR_ALLOC(struct sampling_data_s, data);
	SET_TYPEID(sampling_data_s, data);
