...
#define Op_ADD_LinOP Sop = operator_chain(Sop,NewOp->forward);md_copy_dims(DIMS, CurDims, operator_codomain(Sop)->dims);LinopsVec[opLinopCounter++]=NewOp;
...
complex float* dataFiles[MAX_OpFILES];
long Fdims[MAX_OpFILES][DIMS];

const struct linop_s* LinopsVec[MAX_OPS];
long opLinopCounter=0;

long * getOpFdims(long i) { return Fdims[i]; }
complex float* getOpDataFile(long i) { return dataFiles[i]; }
...
//             printf( "Token: XX%sXX\n", token );
            ...
            if(strcmp(token,"nufft")==0) {
                ...
                if(BasisFileIdx<0 && WeightsFileIdx<0) {
                    NewOp= nufft_create(DIMS, ksp_dims, CurDims, Fdims[TrajFileIdx], dataFiles[TrajFileIdx], NULL, nuconf);
                } else if(BasisFileIdx<0 && WeightsFileIdx>=0) {
                    NewOp= nufft_create2(DIMS, ksp_dims, CurDims, Fdims[TrajFileIdx], dataFiles[TrajFileIdx], Fdims[WeightsFileIdx],dataFiles[WeightsFileIdx],NULL,NULL, nuconf);
                } else if(BasisFileIdx>=0 && WeightsFileIdx<0) {
                    NewOp= nufft_create2(DIMS, ksp_dims, CurDims, Fdims[TrajFileIdx], dataFiles[TrajFileIdx], NULL,NULL, Fdims[BasisFileIdx],dataFiles[BasisFileIdx], nuconf);
                } else if(BasisFileIdx>=0 && WeightsFileIdx>=0) {
                    NewOp= nufft_create2(DIMS, ksp_dims, CurDims, Fdims[TrajFileIdx], dataFiles[TrajFileIdx],
            ...
            if(strcmp(token,"f")==0) {
                ...
                Sop = operator_chain(Sop,LinopsVec[Idx]->forward);
            ...
            if(strcmp(token,"a")==0) {
                ...
                Sop = operator_chain(Sop,LinopsVec[Idx]->adjoint);
                md_copy_dims(DIMS, CurDims, operator_codomain(Sop)->dims);
            }
            if(strcmp(token,"n")==0) {
                token = strtok(NULL, " ,.-");
                ...
                Sop = operator_chain(Sop,LinopsVec[Idx]->normal);
                md_copy_dims(DIMS, CurDims, operator_codomain(Sop)->dims);
            }
		}
	}
